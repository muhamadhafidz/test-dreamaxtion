...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased">
        <form action="/testSoal1" method="POST">
            @csrf
            <h5>Soal nomor 1</h5>
            <input type="text" class="form-control" name="inputanSoal1">
            <button type="submit">Submit</button>
            {{-- check session apakah ada --}}
            @if (session()->get('resultSoal1', '') != '')
                <h4>JAWABANNYA : {{session()->get('resultSoal1', ''); }}</h4>
                {{ session()->forget('resultSoal1') }}
            @endif
        </form>
            <hr>
            <form action="/testSoal2" method="POST">
                @csrf
                <h5>Soal nomor 2</h5>
                <input type="text" class="form-control" name="inputanSoal2">
                <button type="submit">Submit</button>
                {{-- check session apakah ada --}}
                @if (session()->get('resultSoal2', '') != '')
                    <h4>JAWABANNYA : {{session()->get('resultSoal2', ''); }}</h4>
